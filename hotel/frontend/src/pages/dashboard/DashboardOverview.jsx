...
import { OCCUPANCY_LOW_PCT, ROOM_BLOCK_TYPE_LABELS } from '@hotelos/hotel-contracts';
...
/**
 * Günlük durum (modül 13): müdürün sabah baktığı tek ekran — doluluk,
 * gelecek / gidecek, oda durumu, bu gecenin oda geliri ve ADR, haftalık
 * doluluk, bekleyen işler, bugünün kısa listeleri.
 *
...
function KpiGrid({ data, can }) {
  const { today, arrivals, departures, rooms } = data;
  const staysLink = can(PERMISSIONS.STAYS_VIEW);
  const roomsLink = can(PERMISSIONS.ROOMS_VIEW);
  const cleaningBehind = arrivals.unassigned > rooms.vacantReady;
  return (
    <ul className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
      <Kpi
        ...
        tone={today.occupancyPct < OCCUPANCY_LOW_PCT ? 'warning' : 'neutral'}
        lines={[
      ...
      <Kpi
        ...
        tone={arrivals.unassigned > 0 ? 'warning' : 'neutral'}
        lines={[
          `Bugün giriş yapan: ${arrivals.checkedIn}`,
          arrivals.unassigned > 0 ? `${arrivals.unassigned} misafirin odası verilmedi` : 'Hepsinin odası hazır',
        ]}
      ...
      <Kpi
        ...
        lines={[`Hazır boş oda: ${rooms.vacantReady}`, `Toplam ${rooms.total} oda`]}
      ...
      <Kpi
        ...
        // Odası verilmemiş gelecekler hazır boş odadan fazlaysa temizlik yetişmeli.
        tone={cleaningBehind ? 'warning' : 'neutral'}
        lines={[
          cleaningBehind
            ? `Odası verilmemiş ${arrivals.unassigned} misafire ${rooms.vacantReady} hazır oda var: temizlik öncelikli`
      ...
      <Kpi
        ...
        lines={[
          today.adr !== null ? `ADR ${formatMoney(today.adr, data.currency)}` : 'Satış yok, ADR hesaplanmadı',
          'Eldeki rezervasyonun gece fiyatları',
        ]}
      />
    </ul>
  );
...
function Kpi({ icon, label, value, valueHint, lines, tone = 'neutral', to = null }) {
  const body = (
    <>
      ...
      <p className={`mt-2 text-2xl font-bold leading-tight ${TONES[tone]}`}>
        {value}
        {valueHint && <span className="ml-1.5 text-sm font-semibold text-ink-muted">{valueHint}</span>}
      </p>
...
/**
 * Bekleyen işler: onaylar, manuel görevler, açık arızalar. Kişi yalnızca
 * yetkisi olanı görür; özetler sunucuda dakikada bir hesaplanır (her panel sormaz).
 *
        ...
        <li className="py-3">
          <div className="flex items-center gap-3">
            ...
            <span className="min-w-0 flex-1">
              ...
              <span className="block text-xs text-ink-muted">{faults.total === 0 ? 'Bugün arızalı ya da hizmet dışı oda yok' : 'Arızalı oda satılamaz; hizmet dışı oda satışta ama atanmaz'}</span>
            </span>
            ...
            <ul className="mt-2 space-y-1 pl-12 text-xs">
              {faults.items.map((fault) => (
                <li key={fault.id} className="flex flex-wrap items-baseline gap-x-2">
                  ...
                  <span className="text-ink-muted">{fault.endDate ? `${longDay(fault.endDate)} bitiyor` : 'süresiz'}</span>
                </li>
              ))}
...
/**
 * Bugünün gelecek / gidecek kısa listesi; tamamı ön büroda.
